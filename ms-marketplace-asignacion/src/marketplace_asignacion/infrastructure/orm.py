...
class DlqORM(Base):
    """Dead Letter Queue local para mensajes que el producer no pudo publicar."""
    ...
    original_outbox_id: Mapped[Optional[str]] = mapped_column(String(36), nullable=True)
    aggregate_type: Mapped[str] = mapped_column(String(100), nullable=False)
    aggregate_id: Mapped[str] = mapped_column(String(36), nullable=False)
    event_type: Mapped[str] = mapped_column(String(100), nullable=False)
    version: Mapped[int] = mapped_column(Integer, nullable=False, default=1)
    ...
    reprocessed_at: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True)
