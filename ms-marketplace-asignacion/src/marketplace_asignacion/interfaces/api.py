...
def solicitar_trabajo(
    ...
    # NOTA DEMO: El relay del outbox corre en un worker separado (consumidor_demo.py)
...
def acreditar_proveedor(proveedor_id: uuid.UUID):
    """Endpoint de utilidad para demos. Acredita un proveedor en memoria."""
    _acreditacion_adapter.acreditar(proveedor_id)
    return {"detail": f"Proveedor {proveedor_id} acreditado"}


# ── Endpoints de administración DLQ ───────────────────────────────


@router.get("/admin/dlq")
def listar_dlq(db: Session = Depends(get_db)):
    """Lista mensajes en la DLQ local (no publicados a Pulsar)."""
    from marketplace_asignacion.infrastructure.orm import DlqORM

    rows = db.query(DlqORM).order_by(DlqORM.failed_at.desc()).limit(100).all()
    return {
        "count": len(rows),
        "items": [
            {
                ...
                "reprocessed_at": r.reprocessed_at.isoformat()
                if r.reprocessed_at
                else None,
            }
            for r in rows
...
def reintentar_dlq(db: Session = Depends(get_db)):
    """Reintenta publicar mensajes pendientes de la DLQ local."""
    from marketplace_asignacion.infrastructure.pulsar_producer import (
        PulsarEventProducer,
    )

    producer = PulsarEventProducer(session_factory=lambda: db)
    result = producer.retry_dlq(limit=100)
    return {
        "detail": "Reintento de DLQ completado",
